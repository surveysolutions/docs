...
### The protocol for conducting listing and census operations

The feature of assignment status opens the following workflow for conducting the
listing and census operations:

- HQ users create infinite assignments for specific enumeration zones with
specific teams (supervisor accounts) responsible for data collection. Target
area boundaries can optionally be also supplied as a shape file, so that the
interviewers may benefit from the geofencing feature.

- Each supervisor distributes the assignments among the interviewers of their
teams.

- Each interviewer scouts the assigned area and creates new interviews for the
encountered enumeration units (households, persons, enterprises, etc).

- Once the area is exhausted, the interviewer signals about it by changing the
status of the assignment to `COMPLETE`.

- The supervisor reviews the information submitted by the interviewer, possibly
looking at the number of interviews submitted by the interviewer, geotracking
tracks and the comments in the assignment closing transaction and decides
whether to accept the area as completed (close the assignment) or not (reopen
the assignment).

- If a supervisor didn't make a decision within a set time, an HQ user may act
to review the data and close or reopen the assignment for that area.

- Interviewers move to the next enumeration areas once they synchronize all the
data for the current one. (They may still need to come back if any of the
